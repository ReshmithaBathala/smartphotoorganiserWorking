...
app.post("/upload", upload.array("image"), (req, res) => {
  ...
  const pythonProcessDuplicate = spawn("python", [
    "duplicate.py",
    inputFolderPath,
  ]);
...
//   console.log("✅ Image uploaded successfully!"); // Step 1: Confirm image upload

//   // Execute script to remove duplicates
//   console.log("🚀 Running duplicate.py...");
//   const pythonProcessDuplicate = spawn("python", [
//     "duplicate.py",
//     inputFolderPath,
//   ]);
...
//   pythonProcessDuplicate.stderr.on("data", (data) => {
//     console.error(`❌ Error in duplicate.py: ${data}`);
//   });

//   pythonProcessDuplicate.on("close", (codeDuplicate) => {
//     console.log(`✅ duplicate.py exited with code ${codeDuplicate}`);

//     // If duplicate removal is successful, proceed with blur detection
//     if (codeDuplicate === 0) {
//       console.log("🚀 Running blur.py...");
//       const pythonProcessBlur = spawn("python", ["blur.py", inputFolderPath]);
...
//       pythonProcessBlur.stderr.on("data", (data) => {
//         console.error(`❌ Error in blur.py: ${data}`);
//       });

//       pythonProcessBlur.on("close", (codeBlur) => {
//         console.log(`✅ blur.py exited with code ${codeBlur}`);

//         // If blur detection is successful, proceed with location-based sorting
//         if (codeBlur === 0) {
//           console.log("🚀 Running location.py...");
//           exec("python location.py", (error, stdout, stderr) => {
//             if (error) {
//               console.error(`❌ Error in location.py: ${error}`);
//               return;
//             }
//             console.log(`✅ location.py output: ${stdout}`);
//             console.error(`🚨 location.py errors: ${stderr}`);
...
//                 else console.log("✅ Zip file sent successfully!");

//                 fs.unlinkSync(outputZipPath); // Delete zip after sending
//                 removeFolderContents(blurFolderPath);
//                 removeFolderContents(locationFolderPath);
//                 removeFolderContents(inputFolderPath);
//               });
